 ...
 * could never resolve. Wrapped for the same remap as open/stat. */
DIR *opendir_fake(const char *path) {
  char b[320];
  return opendir(sd_remap_path(path, b, sizeof b));
}
...
const char *dlerror_fake(void) {
  ...
  snprintf(msg, sizeof msg, "%s: refused by sonicdash_nx (not available on Switch)",
           s_dl_refused);
  s_dl_refused = NULL;
  return msg;
}
 ...
 * entirely -- `(void)t;`. Against Sonic Dash's libil2cpp those addresses point
