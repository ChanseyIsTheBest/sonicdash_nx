/* sd_home.c -- the game folder: wherever the .nro was launched from.
 *
 * The Homebrew Menu passes the .nro's own path as argv[0], for example
 * "sdmc:/switch/sonic_nx/sonicdash_nx.nro"; that folder holds the game files,
 ...
 * the SD card's root, which is no game folder -- gets SD_DEFAULT_HOME.
 *
...
static char s_home[256] = SD_DEFAULT_HOME;
static const char *s_why = "default -- not decided yet";

const char *sd_home(void) { return s_home; }
const char *sd_home_source(void) { return s_why; }

int sd_home_from_argv0(const char *a0, char *out, size_t n) {
  char buf[512];
  if (!a0 || !*a0) return 0;
...
void sd_home_init(int argc, char **argv) {
  ...
  if (argc > 0 && argv && sd_home_from_argv0(argv[0], h, sizeof h)) {
    snprintf(s_home, sizeof s_home, "%s", h);
    s_why = "the .nro's own folder";
  } else {
    snprintf(s_home, sizeof s_home, "%s", SD_DEFAULT_HOME);   /* the report never disagrees with the folder */
