...
/* Boehm GC stop-the-world bridge: pthread_kill() from libil2cpp lands here and
 * is forwarded to sd_gc.c, which really suspends the target, publishes its stack
 * and registers to GC_threads[], and posts the ack. sd_gc_arm() receives the
 * libil2cpp base directly from main.c; no global carries it. */
int pthread_kill_gc(pthread_t t, int sig);

#include <dirent.h>
DIR *opendir_fake(const char *path);   /* opendir + sd_remap_path */
