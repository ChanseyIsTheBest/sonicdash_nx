...
typedef struct {
  ...
  volatile int          gc_paused;      /* held stopped by the GC bridge          */
} DiagThread;
 ...
 * sd_gc.c needs to stop a thread by pthread_t and read its registers while it
 * is stopped. snapshot_thread() below already does the svcSetThreadActivity
 * dance for the watchdog; these expose it by pthread_t and, crucially, hold
 * the stop across the call rather than resuming immediately.
 *
 * ONE LOCK COVERS BOTH USERS. Without it the watchdog could resume a thread
 * the collector is holding stopped -- it would then run during the mark, which
 * is the exact bug this bridge exists to avoid -- or the collector could
 * "pause" a thread the watchdog is about to resume. Neither side logs or waits
 * while holding it.
 *
 * SAFETY: never pause the caller (that is the collector itself, and pausing it
 * deadlocks instantly); never double-pause, so resume stays symmetric; and
 * report failure rather than hiding it, because the caller still has to ack. */
static Mutex g_pause_lock;

/* Never freeze a thread that is mid-debugPrintf: it holds the stdio/heap lock,
 * and the collector deadlocks against it the moment anything else logs.
 *
 * This tree has no DIAG_WAIT_LOG wait-kind to test (battd_nx's does), so we
 * cannot ask "is it logging?" directly. Rather than invent a constant that
 * nothing sets -- which would compile and silently never wait -- serialise on
 * the log lock itself: if we can take it, nobody is mid-write. Taking and
 * immediately releasing is enough, because a thread that starts logging after
 * this point will block on a lock we are not holding, not on us.
 *
 * If you later add a DIAG_WAIT_LOG beacon, prefer it -- it is cheaper than a
 * lock round-trip per suspended thread per collection. */
static void wait_not_logging(DiagThread *t) {
  (void)t;
  log_lock_barrier();      /* util.c owns the lock; this is the barrier */
}

/* 1 = paused and ctx valid, 2 = paused but no ctx, 0 = not paused. */
void alloc_locks_hold(void);      /* libc_shim.c */
void alloc_locks_release(void);
...
int diag_pause_pthread_ctx(void *target_pthread, ThreadContext *ctx) {
  ...
  for (int i = 0; i < DIAG_MAX_THREADS; i++) {
    ...
    if (!t->gc_paused) {
      wait_not_logging(t);
      /* AFTER wait_not_logging: a target mid-log may need the malloc lock to
       * finish, so holding it before would deadlock the two. Wrapped tightly
       * around the pause: no frozen thread can own an allocator lock. */
      alloc_locks_hold();
      const Result pr_ = svcSetThreadActivity(t->handle, ThreadActivity_Paused);
      alloc_locks_release();
      if (R_FAILED(pr_)) break;
      t->gc_paused = 1;
    }
...
static void snapshot_thread(DiagThread *t) {
  ...
  /* The GC bridge is holding this one stopped. Resuming it here would let it
   * run during the collector's mark phase -- the corruption this whole bridge
   * exists to prevent. Skip it; the collector will resume it. */
  if (t->gc_paused) { debugPrintf("[wd]   %-16s (held by GC)\n", t->name); return; }
  ThreadContext ctx;
...
static void dump_threads(int episode, uint64_t now) {
  ...
  /* A stop-the-world in progress? Name the collector and replay what the bridge
   * did. The first freeze of this kind showed four threads "held by GC" and no
   * way to tell which thread was collecting -- so no way to see where it hung. */
  {
    uint64_t ctid; double secs; unsigned round; int paused;
    if (sd_gc_stall_info(&ctid, &secs, &round, &paused)) {
      const char *cn = "(not a registered thread)";
      for (int i = 0; i < DIAG_MAX_THREADS; i++)
        if (g_threads[i].in_use && g_threads[i].tid == ctid) { cn = g_threads[i].name; break; }
      debugPrintf("[wd] GC STOP-THE-WORLD round %u in progress for %.1fs -- COLLECTOR = %s (tid %llu), "
                  "%d thread(s) paused. The collector's stack below shows where the round is stuck.\n",
                  round, secs, cn, (unsigned long long)ctid, paused);
      for (unsigned b = 0; b < 12; b++) {
        int op, pr; uintptr_t pth; double ago;
        if (!sd_gc_event(b, &op, &pth, &pr, &ago)) break;
        const char *tn = "?";
        for (int i = 0; i < DIAG_MAX_THREADS; i++)
          if (g_threads[i].in_use && (uintptr_t)g_threads[i].pth == pth) { tn = g_threads[i].name; break; }
        debugPrintf("[wd]   gc event -%u: %s %-24s pr=%d  %.1fs ago\n",
                    b, op == 1 ? "suspend" : "resume ", tn, pr, ago);
      }
    }
  }
