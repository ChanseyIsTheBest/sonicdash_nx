...
/* (A donor port's eLanguage picker lived here -- dead in Sonic Dash; the
...
static void *dispatch_object_impl(void *recv, const FakeID *id, va_list va) {
  ...
  /* Sonic Dash first (sd_jni.c): its claims include a class under
...
/* ...A variants and Sonic Dash's claimed classes.
...
#define APP_VERSION_NAME SD_VERSION_NAME  /* "10.3.1" */
#define APP_VERSION_CODE SD_VERSION_CODE  /* Sonic Dash versionCode */
...
static juint field_int(const FakeID *id) {
  ...
  /* ApplicationInfo -- read from Sonic Dash's own manifest (config.h). Unity
